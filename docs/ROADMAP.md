...
대시보드의 "본인 관련 일정 필터링" 핵심 가치를 살리기 위한 작업.
2-A (입력 폼 보강) 와 2-B (대시보드 재설계) 두 단계로 분할.

#### 정리 2-A — SchedulePage 폼 보강 + 부서 통합 + scope 단순화 — ✅ 완료

**구현된 범위**:
- `timetableData.js` 에 `DEPT` 상수 추가 (`['교무부','연구부','학생안전부','학생생활부','진로부','정보부']`).
  App.jsx 의 기존 `DEPT_LIST` 는 이걸 import 해서 사용. 출처 단일화.
- `SchedulePage` EventModal 폼에 입력 필드 추가:
  - 우선순위 (priority): 높음/중간/낮음 3-버튼 토글 (tasks 모달 패턴 재사용)
  - 대상 태그 (tags): 전체 / 담임 / 교과 멀티 셀렉트 체크박스
  - 관련 부서 (dept): 셀렉트박스, 비워둘 수 있음 (기본값 없음)
  - 나만 보기 토글 (가상 필드, DB 저장 X): 체크 시 tags/dept 영역 비활성화 + 저장 시 tags=[] 강제
- 기존 `scope='all' | 'personal'` 토글 제거. 시스템 표현 단순화:
  - `tags=['전체', ...]` → 공유 일정 (대시보드/나의 할 일이 매칭)
  - `tags=[]` (빈 배열) → 개인 일정 (본인만)
  - `scope` 컬럼은 코드에서 사용 안 함. DB 컬럼은 보존 (데이터 손실 없이 마이그레이션 가능).
- 수정 모달 진입 시 기존 값 복원 (tags/dept/priority 모두). tags 가 빈 배열이면 "나만 보기" 자동 체크.
