...
- [x] **모듈 C-2 수업일수·시수 파생** (`lib/school-days.js`, /calendar 의 "수업일수·시수" 뷰)
      캘린더(official)에서 월별 수업일수와 주당 1~5시간별 시수/누계를 계산한다.
      자산 생성기 `scripts/build-fixed-hours.mjs` (기본 읽기 전용, `--write` 로만 갱신).
      ※ doc-ai 파이프라인은 **기존대로 자산만 읽는다** — 시수는 결재 문서에 박히는
        숫자라, 캘린더를 손보는 순간 이미 만든 계획서와 갈리면 안 된다.
        캘린더 = 원천, 자산 = 찍은 시점의 스냅샷
      ※ **파생 시수표와 기존 고정표가 12월 행에서 다르다** (5칸). 원인은 밝혀져 있고
        어느 쪽이 맞는지는 계란님 판정 대기 — `tests/test_school_days.mjs` 의
        KNOWN_DIFF 참조. 판정 후 `--write` 로 자산을 다시 찍는다
- [ ] **라벨 체크리스트 전환** (C-3 때) — 지금은 캘린더 라벨이 자유 입력 + 자동완성이라
      '학사'/'학사일정'/'학사 일정' 처럼 갈라진다. 라벨 사전을 자산으로 두고 체크박스로
      고르게 바꾼다 (자유 입력은 남기되 사전에 없는 라벨은 확인 한 번 거치게).
      대시보드가 라벨로 일정을 골라 보여줘야 하므로 C-3 전에는 정리돼야 한다
