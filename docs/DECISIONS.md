...
> 작성일: 2026-08-14 / 갱신: 2026-08-15 — Phase 0 착수 검토 반영 (D15~D17 추가, P3·P4 보강) / 2026-08-16 — v1 실측 판정 반영 (D3 갱신, 시드 009 확정) / 2026-08-16 — D18 추가 (무료 플랜 한도로 v1 Supabase 프로젝트 초기화 후 재사용, 000_reset 확정. D2 일부 대체)
...
| D3 | 데이터 이관 | (2026-08-16 실측 갱신) 실제 이관 대상은 **부서 6개 명단뿐** (009_seed). v1 업무종류 5건은 샘플로 판정·폐기 (원본 backup/ 보존), 계정은 D17 로 대체. 나머지 전부 폐기 |
...
| D18 | Supabase 프로젝트 조달 | (2026-08-16) 무료 플랜 프로젝트 한도로 신규 생성 불가 → **v1 프로젝트를 000_reset_v1_project.sql 로 초기화해 재사용**. v1 사이트는 운영 중단, DB 백업은 생략(계란님 결정 — 보존 대상은 이미 backup/ 에 확보됨). **v1 GitHub repo 는 v1 의 유일한 참조본이므로 절대 보존.** URL/anon key 는 v1 것을 그대로 사용, Google OAuth 클라이언트와 Supabase Provider 설정도 재사용 |
...
- enrollments 에 UNIQUE (class_group_id, student_no) 추가 여부: 같은 반 번호 중복(엑셀 업로드 오류)을 DB 가 잡아주는 장점 vs 전입생 결번 승계 관행과의 충돌 → 학교 관행 확인 후 005 작성 전(Phase 3)에 결정
