# 🎯 결정 누적 (DECISIONS)

이 문서는 Phase 1 부터 현재까지 결정된 모든 사항을 누적 기록합니다. **앞으로의 작업은 이 결정들을 반드시 따라야 합니다.**

---

## Phase 1 — 데이터 모델 + 시간표 보기 기반

### 데이터 모델
- 4개 핵심 테이블: `timetables`, `timetable_changes`, `school_calendar`, `notifications`
- `timetables.data` 형식: `{ classId: { 'day-period': {sid, tid}, ... } }`
- 시간표 한 번에 하나만 active (UNIQUE INDEX on `is_active`)
- 변동 요청은 별도 테이블 (`timetable_changes`) 로 시간표 본체와 분리

### 시간표 보기 정책
- 학급 뷰 / 교사 뷰 토글
- 주차 단위 네비게이션 (한 번에 한 주)
- 캘린더 오버레이 (휴일/시험/행사 라벨)
- 본인 수업 셀만 클릭 가능 (관리자 모드는 예외)

### 시뮬레이션 페르소나
- Phase 4 (인증) 전까지 임시
- 시드 시간표에 등장하는 6명 + admin
- 기본값 't2' (국어T2)

---

## Phase 2A — 변동 요청 + 2단 승인 + 알림

### 결정 1 — 변동 유형 (4가지)
- `swap` (교환수업) — 다른 시간 수업과 맞바꿈, 1:1/1:1:1/1:1:1:1 가능
- `substitute` (보강) — 다른 교사가 대신 들어감
- `self_study` (결강/자습) — 자습 처리, 감독 교사 지정 가능 (없으면 결강)
- `period_move` (시수변경) — 같은 교사가 시간만 옮김

### 결정 2 — Swap 정책
- **같은 학급 안 swap 이 원칙**
- **다른 학급 swap 도 가능하지만 경고 표시** (노란색 ⚠️)
- 학교 운영 관점에서 같은 학급 안 교환이 학생/학부모 혼란 적음

### 결정 3 — 승인 흐름
- **2단계**: 영향받는 파트너 교사 승인 → 시간표관리자 승인
- partner 가 한 명이라도 거절하면 전체 반려
- 시간표관리자가 거절하면 반려
- 모두 승인 시 즉시 시간표에 반영

### 결정 4 — 알림 표시 위치
- 사이드바 메뉴 옆 빨간 점 (숫자 배지)
- 시간표 페이지 안 알림 탭
- 대시보드 알림은 Phase 6 으로 미룸

### 결정 5 — 알림 갱신 방식
- 페이지 로드 시 + 30초 폴링
- Supabase Realtime 은 Phase 5~6 에서 업그레이드 검토

### 결정 6 — 페이지 구조
- TimetableViewer 안에 탭 4개 (시간표 / 알림 / 내 요청 / 관리자만 승인 대기)

---

## Phase 2B — AI 추천 + 시간표 전용 AI 비서

### 결정 1 — API 엔드포인트 구조
- 별도 엔드포인트들로 분리 (단일 chat.js 에 통합 안 함)
- `/api/recommend-substitute`, `/api/recommend-move`, `/api/timetable-chat`

### 결정 2 — AI 추천 진입점
- 폼 안에 "✨ AI 추천받기" 별도 버튼
- 첫 진입 시 자동 호출 안 함 (사용자가 직접 선택 가능, 토큰 절약)

### 결정 3 — 채팅 위치
- 우측 사이드 패널 (토글 가능)
- 모바일은 우측에서 슬라이드 인/아웃 (768px 미만)

### 결정 4 — 일반 교사 AI 권한 범위

**할 수 있는 일**:
- 시간표 조회 (본인 + 다른 교사)
- 변동 요청 흐름 안내
- 보강/이동 추천 (변동 폼에서)
- 변동 통계 (조회만)
- 본인 변동 안내문 초안

**할 수 없는 일** (시스템 프롬프트로 거절):
- 시간표 직접 수정
- 다른 교사 변동 대신 만들기
- 시수 검증, 학교 전체 분석
- 시험기간 시간표 생성

---

## Phase 3A — 일반/관리자 모드 분리

### 결정 1 — 직권 변경 진입 방식
- **명시적 모드 토글** 로 일반/관리자 모드 전환
- 페르소나가 시간표관리자일 때만 토글 노출
- 일반 교사 페르소나는 항상 교사 모드

### 결정 2 — 학사일정 vs 일정관리
- **학사일정** (`school_calendar`): 학교 운영 차원 (휴일/시험/행사)
- **일정관리** (`schedules`): 개인/부서 업무 (이전 채팅 작업, 별도)
- 학사일정은 별도 메뉴 "📆 학사일정", 관리자만 접근

### 결정 3 — AI 비서 권한 분기
- 같은 사이드 챗봇, 시스템 프롬프트와 tool 만 페르소나에 따라 다름
- 시각적 구분: 관리자 모드일 때 보라색 헤더 + "관리자 AI" 라벨

### 결정 4 — 직권 변경 사후 통보 범위
- **영향받는 교사들에게만** (담임/부서장 등은 포함 안 함)
- 알림 kind: `admin_notice` (응답 버튼 없음, 단순 통보)

### 결정 5 — 학사일정 4가지 유형
- `holiday` (휴일)
- `no_school` (재량휴업일)
- `exam` (시험)
- `event` (행사) — 일부 학급만 영향 가능 (affected_classes)

---

## Phase 3B — 관리자 AI 비서

### 결정 1 — Tool use 방식
- 본격 tool use (시간 걸려도 정확)
- Claude 가 도구 호출 → 서버 실행 → 결과 다시 Claude 에 → 최종 답변
- max 5 회 루프

### 결정 2 — 변경 제안 처리
- AI 가 시간표 직접 수정 안 함
- `propose_change` 도구가 정보만 반환
- UI 가 보라색 카드로 표시 → 사용자 클릭 시 그제야 적용

### 결정 3 — 채팅 인터페이스 시각적 구분
- 같은 사이드 챗봇이지만 헤더 색상/아이콘/라벨 변경
- 관리자 모드: 보라색 + "관리자 AI 비서" + ⚙️ 아이콘

### 결정 4 — 시수 데이터 출처
- `timetableData.js` 의 `gh` (학년별 표준 시수) + `as` (교사 학급 배정) 그대로 사용
- Phase 5+ 에서 DB 로 옮길 예정

### 결정 5 — 도구 5개
1. `validate_timetable` — 시수/충돌/교사시수 검증
2. `query_change_stats` — 변동 통계
3. `analyze_substitute_load` — 보강 부담 분석
4. `find_available_teachers` — 빈 교사 찾기
5. `propose_change` — 변동 제안

---

## Phase 3C — AI 보조 운영 기능

### 결정 — Phase 3C 보류
- AI 기능 추가는 시범운영 후 정리하여 재설계
- 이미 만든 AI 기능 (Phase 2B + 3B) 만으로 시범운영 가능
- 학교 현장 피드백 받은 후 어떤 AI 가 진짜 필요한지 결정

---

## Phase 4A — 솔버 결과 Supabase 저장

### 결정 1 — 기존 TimetablePage 처리
- A. **점진적 통합**: 기존 페이지 그대로 두고 "Supabase 에 저장" 버튼만 추가
- Phase 5+ 에서 새 페이지가 흡수

### 결정 2 — 솔버 결과 저장 형식
- A. **새 스키마로 변환해서 저장**
- 다행히 솔버 출력 = 우리 스키마 (변환 불필요)

### 결정 3 — 활성화 정책
- B. **드래프트 저장 → 명시적 활성화 버튼**
- 즉시 활성화 옵션도 있지만 기본은 드래프트

### 결정 4 — 편집 기능 범위
- A. **셀 단위 편집만** (Phase 4C 에서 구현)

### 결정 5 — Phase 4 단계 나누기
- 4A → 검증 → 4B → 검증 → 4C → 검증

### 결정 6 — 자동 활성화
- A. **명시적 활성화만**
- 발효일(`effective_from`) 도달 시 자동 활성화 안 함 (cron 인프라 후순위)

---

## Phase 4B — 시간표 목록 / 활성화 관리

### 결정 1 — 시간표 관리 위치
- B. **별도 페이지로 승격**
- 사이드바에 "🗂️ 시간표 목록" 메뉴 추가

### 결정 2 — 이전 시간표 보기
- B. **클릭 시 읽기 전용 보기**
- 활성/드래프트/이전 모두 클릭 가능
- 읽기 전용: 셀 클릭 차단, AI 토글 없음, 캘린더는 활성 시간표일 때만 표시

---

## Phase 4C — 셀 단위 편집 (예정)

### 결정 4 — 편집 대상
- B. **드래프트만 편집 가능**
- 활성 시간표는 직접 편집 불가 (임시 변경 시스템으로 처리)

### 결정 5 — 편집 작업 단위
- C. **단일 셀 편집 + swap 둘 다**
- 단일: 교사/과목 변경 (예: 김국어T1 → 김국어T2)
- swap: 두 셀 내용 교환

### 결정 6 — 충돌 검증 강도
- **확실한 충돌만 강제 차단**: 한 교사 동시간 두 학급, 한 학급 한 시간대 두 과목
- **시수 불일치는 경고만**: 편집 중간 단계에서 일시적 부족/초과 허용
- 합반 케이스 없음 (이 학교) 확인됨

### 결정 7 — 변경 추적
- A. **edit_log 누적 기록**
- 어떤 셀이 어떻게 바뀌었는지 모두 기록
- Phase 7 (영구 변경 시스템) 의 기반

### 결정 8 — 편집 인터랙션 (카드 바구니 방식, 4C-1 결정 5 갱신)

기존 결정 5(단일 셀 편집 + swap 둘 다)는 카드 바구니 UX 로 통합됨.

- **메탈 모델**: 시간표 = 서랍, 학급-과목-교사 묶음 = 카드
- 편집 페이지에 "카드 바구니" 영역 (그리드 하단)
- **셀 클릭 → 카드가 바구니로 이동**
- 바구니의 **카드 클릭(선택) → 빈 셀 클릭 → 배치**
- 점유된 셀에 카드 배치 시: **그 셀의 카드가 자동으로 바구니로 이동** (자동 교환, swap 흡수)
- 빈 셀 클릭(선택 카드 없을 때): 작은 메뉴 "+ 새 카드 추가" → 모달에서 과목/교사 선택
- 카드 식별: 출신 정보 작게 표시 ("원래: 1-2 / 월3")
- 바구니 크기 무제한
- **저장 시 바구니가 비어있어야 가능** (변경 사항 있으나 바구니 차있으면 저장 차단 + 안내)
- 클릭 방식만 지원 (드래그앤드롭 미지원 — 정확도/모바일 호환성)
- 바구니의 카드는 우상단 ✕ 버튼으로 삭제 가능 (confirm 필요)
- 카드 삭제는 edit_log 에 별도 이벤트 추가하지 않음 (이미 기록된 cell_clear 또는 상쇄되는 cell_edit 로 충분)
- 카드 선택 시, 그 카드의 교사가 동시간에 다른 학급에 배정된 셀은 빨간색으로 표시 + 클릭 차단 (결정 6 의 "확실한 충돌" 을 사전 차단으로 격상)
- 시수 초과는 여전히 경고만 (차단 X)

기존 swap 액션(4C-2 예정)은 본 결정으로 흡수되어 작업 큐에서 제거.
**4C-2 는 충돌 강제 차단만 남음.**

---

## Phase 5-A — 학교 정적 데이터 DB 화

### 결정 1 — DB 스키마
- 신규 테이블 4개: `subjects` (TEXT PK), `classes` (TEXT PK), `departments` (부서명 PK), `teacher_assignments` (UUID PK + UNIQUE 3-tuple).
- `teachers` 에 컬럼 4개 추가: `homeroom_class_id`, `day_restriction (INTEGER[])`, `is_placeholder`, `is_external`.
- 모든 신규 테이블 RLS 비활성화 (007 사고 재발 방지, Phase 6 와 일괄 RLS 설계).

### 결정 2 — ID 정책 (D6)
- `teachers.id` 영원히 TEXT.
- placeholder 행 (t1~t22, ti, ts3) 과 실 사용자 행 (UUID 형식 TEXT) 공존.
- **Phase 6 의 "UUID 컬럼 복원 (003 역방향)" 결정 폐기**. 모든 사용자 ID 컬럼 영구 TEXT.
- 근거: 003 이 손댄 5개 컬럼만 UUID 로 되돌리면 `teachers.id` (TEXT) 와 타입 불일치 + placeholder 매핑 불가. JSONB 내부 ID (timetables.data, payload) 의 마이그레이션 비용도 큼.

### 결정 3 — wrapper 패턴 (D3, C-2)
- Phase 5-A 는 **순수 시드만**. 코드 변경 0.
- 진실의 원천 = `src/lib/timetableData.js` 상수, DB = mirror.
- 16개 import 사이트 모두 동기 API 그대로 동작.
- Phase 5-B 에서 학교 설정 편집 UI 부활 시 진실의 원천을 DB 로 전환 (그때 wrapper 패턴 정확한 옵션 재논의).

### 결정 4 — homeroom 컬럼 처리 (D1)
- `homeroom_class_id` 신규 컬럼 (class_id 형식, 예: 'c7').
- 기존 `homeroom` 컬럼 보존 (DROP 안 함, 정리 2-A 의 `events.scope` 패턴과 동일).
- 표시명은 향후 `homeroom_class_id` → `classes` 테이블 lookup 으로 동적 계산 (Phase 5-B 작업).

### 결정 5 — 교사 시드 전략 (D2)
- TCH 24명을 `teachers` 테이블에 **직접 시드** (별도 `staff_profiles` 테이블 만들지 않음).
- `is_placeholder=TRUE`, `user_id=NULL`, `status='approved'`.
- `ON CONFLICT (id) DO NOTHING` — 기존 Google 사용자 행 보호.
- Phase 6 인증 통합에서 실 사용자가 placeholder 의 `user_id` 를 채우며 매핑 (id 보존).

### 결정 6 — TCH.as 구조 (D4)
- 별도 `teacher_assignments` 테이블. `(teacher_id, subject_id, class_id)` UNIQUE.
- 정규화 + Phase 5-B 편집 UI 가 행 단위 CRUD 자연스러움.

### 결정 7 — TCH.al 구조 (D5)
- `teachers.day_restriction INTEGER[]` 단일 컬럼.
- 별도 테이블 없음 — 보통 0~2개 값, 정규화 가치 낮음.

### 결정 8 — DB 화 범위
- ✅ 시드 대상: SBJ / CLS / TCH / DEPT.
- ❌ 코드 유지: DAYS / DP / DAILY / SP / D2N / TIMES / CLR (운영 규칙·시각 — 학교 차원 거의 불변).
- ❌ 코드 유지: 순수 함수 (enc / decS / isV / getSP / gS / gC / gT / getHR).

---

## Phase 5-A 정비 — DB 잔재 정리 (2026-05-27)

Phase 5-A 코드 작업 (커밋 `b2d05ce`) 완료 후 DB 점검에서 발견된 이전 작업자의 광범위한 잔재를 정리. 마이그레이션 `010_phase5a_cleanup.sql`.

### 발견된 잔재의 정체

이전 작업자가 시간표 솔버 DB 화를 시도하며 남긴 잔재:

- `teachers.id` 가 UUID 타입 — 003 마이그레이션 정책 + D6 결정 양쪽 모두와 충돌. 009 시드의 `t1~t22` placeholder INSERT 가 이 때문에 실패한 원인.
- `teachers` 잔재 컬럼 3개: `is_homeroom (bool)`, `is_parttime (bool)`, `allowed_days (jsonb)`. Phase 5-A 의 신규 컬럼 (`homeroom_class_id` / `day_restriction` / `is_external`) 과 의미 중복. 코드 전체에 참조 0건.
- 고립 잔재 테이블: `teacher_constraints` (코드 참조 0건, FK CASCADE).
- 의존 FK 3개 (`teacher_constraints.teacher_id`, `events.created_by`, `documents.uploaded_by`) — 모두 teachers.id 참조 + UUID.
- 이미 DROP CASCADE 된 구 잔재 (Phase 5-A 작업 중 정리됨): 구 subjects/classes/teacher_assignments (UUID PK), weekly_hours.

### 진단 결과 (계란님 Supabase 확인)

- `timetables.data` 의 `tid` 형식: `t2, t4, t7, t12, t14, t20` (TEXT) — 매핑 작업 자체가 불필요.
- `events.created_by` 1건: 계란님 본인 UUID `7ac5cbc9-...` — TEXT 캐스트 시 같은 값 보존.
- `teacher_constraints` 0건, `documents.uploaded_by` 0건 — 캐스트/DROP 부담 없음.

### 결정 1 — D6 유지

`teachers.id` 영원히 TEXT. UUID 유지 + `placeholder_code` 매핑 컬럼 추가 옵션은 폐기.

근거:
- `timetables.data` JSONB 와 솔버 출력이 이미 `t1~t22` 형식을 씀 → TEXT 가 자연 매핑.
- 코드 전체가 teacher.id 를 그저 문자열로 비교 (`teachers.find(t => t.id === tid)`) — DB 컬럼 타입 변경의 코드 영향 거의 0.
- 의존 FK 3개 중 사실상 `teacher_constraints` 1개만 진짜 충돌, 나머지 2개는 데이터 0~1건 + 비즈니스 로직 무관.

### 결정 2 — 잔재 컬럼 DROP

`teachers.is_homeroom / is_parttime / allowed_days` 모두 DROP. 기존 `homeroom` (text) 컬럼은 보존 (Phase 5-A 결정 4 의 D1 패턴 유지).

근거: 코드 참조 0건. Phase 5-A 의 신규 컬럼이 의미상 상위 호환. 통합 시도하면 신규 컬럼 의미가 흐려짐.

### 결정 3 — teacher_constraints 테이블 DROP

`teacher_constraints` 테이블 자체 삭제. 마이그레이션 011 이후로 재등장 금지.

근거: 코드 참조 0건. 솔버는 `timetableData.js` 의 `TCH` 상수에 모든 제약 (al, hc 등) 을 내장 → 별도 테이블 필요 없음.

### 결정 4 — 의존 FK 3개 동시 변경 + 재생성

`events.created_by`, `documents.uploaded_by` 의 UUID 컬럼도 TEXT 로 변경. FK 는 원래 정책 (`ON DELETE SET NULL`) 으로 재생성.

`teacher_constraints` 의 FK 는 테이블 자체가 DROP 되므로 재생성 없음.

### 결정 5 — 기존 데이터 모두 보존

`ALTER COLUMN TYPE USING ... ::text` 는 UUID 값을 같은 문자열로 캐스트 (예: `7ac5cbc9-...`). 어떤 행도 손실되지 않음.

`teachers` 1행 (계란님), `events` 1행 (계란님 일정) 그대로 보존.

### 트랜잭션 안전성

`010_phase5a_cleanup.sql` 은 한 BEGIN/COMMIT 으로 묶여 6단계 (FK drop → 잔재 테이블 drop → teachers.id TEXT → 의존 컬럼 TEXT → 잔재 컬럼 drop → FK 재생성) 모두 성공해야만 적용. 한 단계라도 실패하면 자동 ROLLBACK.

### D6 정합성 재확인

010 적용 후의 DB 상태는 D6 결정과 완전히 일치:
- `teachers.id` 영구 TEXT.
- placeholder (`t1~t22`, `ti`, `ts3`) + 실 사용자 (UUID 형식 TEXT) 공존.
- 모든 사용자 ID 컬럼 (`requester_id`, `created_by`, `uploaded_by` 등) TEXT.
- Phase 6 의 UUID 복원 결정은 폐기 상태 유지 (이 문서 "향후 결정 필요 사항" 참조).

### 후속 작업

- 010 적용 직후 `009_seed_school_data.sql` 재실행 → placeholder 24명 + 시수 123건 채워짐.
- 코드 변경 0 (D3 = C-2 정책 유지). `src/` 디렉토리는 손대지 않음.
- Phase 5-B (학교 설정 페이지 부활) 가 이 정비된 상태를 전제로 진행.

---

## 정리 작업 2-C — 시간표 영역 통합 정돈

### 결정 1 — 사이드바 메뉴 구조 단순화
- 기존 3개 시간표 메뉴 (`timetable` / `timetable_v2` / `timetables_list`) → 2개로 축소.
- `📅 시간표` (TimetableViewer) — **commonMenus 로 승격**. 일반 교사도 사이드바에서 직접 진입 가능.
- `🗂️ 시간표 관리` (TimetablesListPage) — adminMenus 유지. 드래프트/활성화/편집/이력 행정 작업.
- `🗓️ 시간표 관리` (TimetablePage 솔버 페이지) 메뉴 폐지.
- 일정 관리 아이콘만 📅 → 🗓️ 로 변경 (시간표와 충돌 해소).

### 결정 2 — 솔버는 시간표 관리 페이지의 2-스텝 모달
- 사이드바 별도 메뉴 없음. `🗂️ 시간표 관리` 헤더의 `+ 새 드래프트 만들기` 버튼이 진입점.
- Step 1 — 초기설정 미리보기 (read-only). "이 데이터로 만든다" 사전 확인.
- Step 2 — 솔버 실행 + 결과 미리보기 + 통계 + 드래프트 저장.
- 결과는 모달 닫을 때까지 보존. Step 1 ↔ 2 자유롭게 이동.
- **항상 드래프트로 저장** (asActive=false). 활성화는 목록 페이지의 별도 버튼.
- 모달 외부 클릭 / ESC: 결과 있으면 confirm 으로 보호.

### 결정 3 — TimetablePage 파일 보존 방식
- 파일 자체 삭제하지 않고 deprecated stub (`export default () => null`) 으로 교체.
- 사유: git history 보존 + 향후 참조용. 822줄짜리였던 원본의 각 탭이 어디로 이동했는지 주석으로 안내.
- 완전 삭제는 Phase 5 또는 Phase 7 정리 시점에.

### 결정 4 — TimetablePage 의 잔존 탭 4개의 운명
- `📅 시간표 보기` 탭 — TimetableViewer 의 메인 화면으로 흡수 (이미 정식 구현).
- `🔄 교체 요청` 탭 — 폐지 (로컬 useState 데모용. 정식 흐름은 TimetableViewer 의 셀 클릭 → ChangeRequestForm).
- `🔄 교체 관리` 탭 — 폐지 (TimetableViewer 의 "승인 대기" 탭과 동일 의도).
- `⚙️ 초기설정` 탭 → `InitSettingsView` 로 분리, SolverModal Step 1 에서 임베드 (read-only).
- `📊 통계` 탭 → SolverModal Step 2 의 접이식 "상세 통계" 섹션으로 흡수 (저장 직전 검증용).
- `✨ 시간표 생성` 탭 → SolverModal Step 2 의 메인.

### 결정 5 — 컴포넌트 분리 규칙
- `src/components/timetable/` 하위에 시간표 영역 신규 컴포넌트 정리.
- 다른 곳에서 재사용되는 ChatView 와 동일한 패턴 (`src/components/ChatView.jsx`).
- 페이지 (`src/pages/`) 와 재사용 컴포넌트 (`src/components/`) 분리.

---

## 정리 작업 2-B — 대시보드 재설계 + 메모장

### 결정 1 — 레이아웃: 좌우 분할 (B)
```
┌─────────────────┬─────────┐
│ 오늘 내 수업       │ 메모장   │
├─────────────────┤         │
│ 내 일정          │         │
│                 ├─────────┤
│                 │ AI 비서  │
└─────────────────┴─────────┘
```
- 왼쪽: 메인 (오늘 수업 + 내 일정), 오른쪽: 사이드 (메모장 + AI 비서)
- 좁은 화면(< 900px) 위아래 스택 자동 전환 — `window.matchMedia` + React state

### 결정 2 — 영역별 표시 방식
- **2-1 오늘 내 수업** (A — 시간순 카드 리스트): 활성 시간표에서 본인 셀 추출, 1교시부터 시간순
- **2-2 내 일정** (B — 오늘 + 이번 주 두 섹션): events 본인 매칭, 정리 2-A 카드 시각 재사용
- **2-3 메모장**: 결정 3 참조
- **2-4 AI 비서** (A — ChatView 페이지 임베드): 기존 ChatView 컴포넌트 그대로, 사이드바 메뉴는 유지

### 결정 3 — 메모장
- **저장 방식 (B)**: DB 신규 테이블 `notes` (user_id PK, content, updated_at)
  - 학교 PC ↔ 집 PC 이동 시 영속성 보장. RLS 는 Phase 6 와 함께 설정.
- **메모 개수 (A)**: 단일 메모 (사용자당 1개) — 대시보드 위젯의 단순성 유지
- **텍스트 형식 (A)**: Plain text — 마크다운 학습 부담 없음
- **저장 트리거 (A)**: 자동 저장 (debounce 1.5초) + "✓ N분 전 저장됨" 표시
- **UI (A)**: 위젯 한 칸에 textarea + 메타 정보 (확대 모달 없음, 시범운영 후 보강 검토)

### 결정 4 — ChatView 분리 방식
- `src/components/ChatView.jsx` 로 별도 파일 분리 (옵션 1).
- 두 곳에서 import:
  * 사이드바 메뉴 (case "chat") — 큰 화면 단독 페이지
  * 대시보드 오른쪽 하단 (DashboardPage) — `compact` prop 으로 위젯 모드
- compact 모드: 헤더 축소, 예시 질문 숨김, "큰 화면 ↗" 버튼 노출 (클릭 시 onOpenFull → setPage('chat'))

---

## 정리 작업 2-A — 일정 시스템 보강 (입력 폼 + scope 단순화)

### 결정 1 — 부서 목록 출처 단일화
- `src/lib/timetableData.js` 의 `DEPT` 상수가 부서 목록의 단일 출처.
- App.jsx, SchedulePage 모두 거기서 import.
- SBJ/TCH/CLS 와 같은 학교 정적 데이터 패턴. Phase 5+ DB 화 시 함께 이동.

### 결정 2 — 일정 공개 범위는 tags 로만 표현 (scope 폐기)
- 기존 `scope='all'|'personal'` 토글 제거.
- 새 규칙:
  - `tags=['전체', ...]` → 공유 일정 (대시보드/나의 할 일 매칭 대상)
  - `tags=[]` (빈 배열) → 개인 일정 (본인만)
- `events.scope` 컬럼은 DB 에 보존 (drop 안 함, 데이터 손실 방지).
- 코드는 더 이상 scope 를 읽거나 쓰지 않음.
- 마이그레이션 006: `scope='all'` 이면서 tags 비어있는 일정에 `tags=['전체']` 자동 보강.

### 결정 3 — 일정 입력 폼 필드 구성
- 우선순위 (priority): 높음/중간/낮음, 기본 '중간'
- 대상 태그 (tags): 전체 / 담임 / 교과 멀티 셀렉트, 기본 ['전체']
- 관련 부서 (dept): 셀렉트, 비워둘 수 있음 (기본 없음)
- "나만 보기" 토글: 가상 필드 (DB 저장 X). 체크 시 tags/dept 영역 비활성화 + 저장 시 tags=[] 강제.
- 수정 모달 진입 시 tags 빈 배열 → "나만 보기" 자동 체크 (추론).

### 결정 4 — 대시보드 필터링 로직 그대로 유지
- 정리 1 보강에서 만든 `e.dept && e.dept===teacher.dept` 방어 + tags 매칭 로직 유지.
- 2-A 는 데이터를 채우는 단계, 2-B 에서 매칭 UX 확정.

---

## 학교 운영 정책 (절대 변경 금지)

이 학교의 운영 정책으로 결정된 사항들. 코드 작업 시 반드시 반영:

1. **합반 없음** — 한 교사가 동시간 두 학급에 들어갈 수 없음
2. **같은 학급 swap 우선** — 다른 학급 swap 도 가능하나 경고 필요
3. **직권 변경 즉시 적용** — 관리자 직권 변경은 승인 단계 없음
4. **사후 통보는 영향받는 교사만** — 담임/부서장 포함 안 함
5. **시간표 한 번에 하나만 active** — UNIQUE INDEX 보장
6. **드래프트 → 명시적 활성화** — 자동 활성화 안 함
7. **변동은 새 timetable row 생성** — 활성 시간표 직접 수정 안 함
8. **모든 변경 추적** — edit_log 누적 기록

---

## 향후 결정 필요 사항 (보류)

다음 Phase 에서 결정해야 할 것들:

### Phase 5 — 임시교사 시스템
- 임시교사 등록 방법
- 진짜 교사 회원가입 시 매핑 UI
- 매핑 시 시간표 데이터 인계 방식

### Phase 6 — 인증 통합
- 페르소나 셀렉터 제거 시점
- ~~UUID 컬럼 복원 마이그레이션 (003 의 역방향)~~ → **Phase 5-A D6 결정으로 폐기**. 모든 사용자 ID 컬럼 영구 TEXT.
- RLS 정책 설계 — `auth.uid()::text` 캐스팅으로 TEXT 비교
- 'admin' fallback 제거 정책
- placeholder 행 (t1~t22, ti, ts3) 의 `user_id` 채워서 실 사용자와 매핑 (id 보존)

### Phase 7 — 영구 변경 시스템
- 영구 변경 등록 UI
- 임시 변경 vs 영구 변경 충돌 검토
- 롤백 절차
- 시간 여행 (과거 시점 시간표 조회)

### Phase 8 — AI 재설계
- 시범운영 피드백 정리
- 어떤 AI 기능을 유지/제거/추가/정교화
- 자동 알림 (보강 부담 모니터링 등) 인프라
- 안내문 일괄 생성 / 변동 다건 영향 분석 등 (Phase 3C 보류 항목)

---

## 결정 변경 시 절차

위 결정을 바꿔야 할 일이 생기면:

1. 사용자와 다시 논의
2. 새 결정에 따라 이 문서 업데이트
3. 영향받는 코드 식별
4. 마이그레이션 또는 점진적 변경 계획

**임의로 결정을 바꾸지 말 것.**
