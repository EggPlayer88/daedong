...
function buildHoursDoc(table) {
  ...
  L.push(
    ...
    '',
    '### 시수는 이렇게 묻는다 (학기 총 시수와 헷갈리지 않게)',
    `- **"학급별 주당 수업 시수를 알려주세요 (${keys[0]}~${keys[keys.length - 1]}시간, 예: 수학 4)"**`,
    '- "시수" 만 물으면 교사가 학기 총 시수(15시간 등)로 답하는 일이 있다.',
    `  **주당**·**시간 단위**·범위·예시를 문장 안에 함께 넣는다.`,
    `- 교사가 ${keys[keys.length - 1]}보다 큰 값을 말하면 (예: 15, 68) 그대로 받지 말고 되묻는다:`,
    '  "주당 시수가 맞을까요? 학기 전체 시수를 말씀하신 것 같습니다. 주당 몇 시간인지 알려주세요."',
    '  학기 총 시수를 주당으로 넣으면 누계가 통째로 어긋난다.',
    '',
