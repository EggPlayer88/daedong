...
ck('활동은 [유지/변경/신규] 3분기 — 유지는 옮겨 적지 않고 참조한다', () => {
  A(fdoc.includes('[유지 / 변경 / 신규]'), '분기 없음')
  A(fdoc.includes('그대로 갈까요, 바꿀까요, 새로 만들까요?'), '질문 문구 없음')
  // 2026-08-26: "요약하지 마라" 라는 당부를 **참조 계승**으로 바꿨다.
  //   당부는 모델이 어길 수 있지만, 참조는 서버가 원문을 넣으므로 어길 수가 없다.
  A(fdoc.includes('carry: ["free_activities[0]"]'), '유지 = 참조 지시 없음')
  A(fdoc.includes('옮겨 적을 원고가 아니다'), '원문의 용도(요약용) 명시 없음')
  A(fdoc.includes('통째로'), '활동 일부만 참조하는 것을 막지 않음')
  A(fdoc.includes('서버가 읽는다'), '☑ 해석 주체가 서버임을 밝히지 않음')
})
