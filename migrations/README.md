...
## archive/ — 실행하지 않고 접은 파일

여기 있는 파일은 **실행 대상이 아니다.** 번호를 다시 쓰지 않으려고 남겨 두는 것이 아니라,
안에 든 **원본 데이터가 아까워서** 남긴다. 되살릴 때는 그대로 실행하지 말고 새 번호로 다시 쓴다.

| 파일 | 접은 날 | 사유 |
|------|---------|------|
| `archive/010_seed.sql` | 2026-08-26 | **구 마스터플랜 시드, 미실행.** 8/16 초기 설계 세션 산출물이라 선행 가정(008=academic_terms)이 현 번호 체계(006=calendar)와 어긋난다. DB 확인 결과 **미실행 확정** — `departments` 테이블은 001 이 만들어 존재하나 **0행**(시드가 들어간 적 없음). **부서 명단 원본은 가치가 있으므로 파일째 보존**한다. 부서 모듈 착수 시 **새 번호로 재설계**해서 실행한다 |
